"""Step 4R-C clinical v2 — Frozen Korean SBERT embeddings on v2 corpus.

Mirror of ``build_step4rc_text_embeddings_clinical.py`` (v1). Only input /
output paths differ. v1 script + v1 npz 는 수정되지 않는다.

Reads (read-only):
    data/step4r/4rc_contrastive_optional/text_corpus_clinical_v2.csv

Writes (new files only):
    data/step4r/4rc_contrastive_optional/text_embeddings_clinical_v2.npz
    data/step4r/4rc_contrastive_optional/text_unique_similarity_clinical_v2.csv
...
Run:
    & C:\\Users\\user\\anaconda3\\envs\\dl_env\\python.exe -X utf8 \\
        scripts/build_step4rc_text_embeddings_clinical_v2.py
"""
...
INPUT_CSV = (
    ...
    / "text_corpus_clinical_v2.csv"
)
OUTPUT_DIR = PROJECT_ROOT / "data" / "step4r" / "4rc_contrastive_optional"
OUTPUT_NPZ = OUTPUT_DIR / "text_embeddings_clinical_v2.npz"
OUTPUT_SIM_CSV = OUTPUT_DIR / "text_unique_similarity_clinical_v2.csv"
...
def main() -> int:
    print("=" * 64)
    print("Step 4R-C clinical v2 — frozen ko-sroberta-multitask embeddings")
    print("=" * 64)
    ...
    print(f"loaded n={len(df)} rows from text_corpus_clinical_v2.csv")
