"""Step 4R-C clinical v2 — Contrastive projection learning on v2 corpus.

본 스크립트는 ``train_step4rc_contrastive_clinical.py`` (v1) 의 v2 분기.
*모든 hyperparameter 동일* (lr 1e-3, batch 256, temp 0.07, seed 42, max 50ep,
patience 10) — 공정한 A/B 비교를 위해 corpus 입력만 swap.

기존 v1 script / v1 checkpoint / v1 joint embeddings 는 수정되지 않는다.
학습 종료 후 v1 joint embeddings 를 *추가로 로드* 해서 같은 retrieval metric
을 재계산하고, v1 vs v2 비교표를 results_clinical_v2.md 에 함께 기록한다.

Reads (read-only):
    data/step4r/4rc_contrastive_optional/imu_embeddings.npz       (frozen 4R-B)
    data/step4r/4rc_contrastive_optional/text_embeddings_clinical_v2.npz   (NEW)
    data/step4r/4rc_contrastive_optional/text_corpus_clinical_v2.csv       (NEW)
...
Run:
    & C:\\Users\\user\\anaconda3\\envs\\dl_env\\python.exe -X utf8 \\
        scripts/train_step4rc_contrastive_clinical_v2.py
"""
...
INPUT_TEXT = PROJECT_ROOT / "data" / "step4r" / "4rc_contrastive_optional" / "text_embeddings_clinical_v2.npz"
INPUT_CORPUS = PROJECT_ROOT / "data" / "step4r" / "4rc_contrastive_optional" / "text_corpus_clinical_v2.csv"
V1_JOINT_NPZ = PROJECT_ROOT / "data" / "step4r" / "4rc_contrastive_optional" / "joint_embeddings_clinical.npz"

OUTPUT_CKPT_DIR = PROJECT_ROOT / "data" / "step4r" / "4rc_contrastive_optional" / "model_clinical_v2"
...
OUTPUT_CKPT = OUTPUT_CKPT_DIR / "projection_head_clinical_v2.pt"
OUTPUT_JOINT_NPZ = OUTPUT_DATA_DIR / "joint_embeddings_clinical_v2.npz"
OUTPUT_LOG_CSV = OUTPUT_REPORT_DIR / "training_log_clinical_v2.csv"
OUTPUT_REPORT_MD = OUTPUT_REPORT_DIR / "results_clinical_v2.md"

# Hyperparameters — v1 과 *완전 동일*
SEED = 42
PROJECTION_DIM = 128
HIDDEN_DIM = 128
IMU_INPUT_DIM = 128
...
def main() -> int:
    print("=" * 64)
    print("Step 4R-C clinical v2 — contrastive projection learning")
    print("=" * 64)
    ...
    for s_name, s_idx in [("train", train_idx), ("val", val_idx), ("test", test_idx)]:
        ...
        final_metrics_v2[s_name] = m
        print(
            f"  v2  {s_name:5s}  strict R@1={m['strict_i2t_R@1']:.4f}  R@5={m['strict_i2t_R@5']:.4f}  R@10={m['strict_i2t_R@10']:.4f}  |  "
    ...
    if V1_JOINT_NPZ.exists():
        ...
        v1_imu = v1["imu_proj"]
        v1_text = v1["text_proj"]
        v1_tpl = v1["template_key"].astype(str)
        assert (v1_sid == sample_id).all(), "v1/v2 sample_id mismatch"
        ...
        for s_name, s_idx in [("train", v1_train_idx), ("val", v1_val_idx), ("test", v1_test_idx)]:
            ...
            print(
                f"  v1  {s_name:5s}  strict R@1={m['strict_i2t_R@1']:.4f}  R@5={m['strict_i2t_R@5']:.4f}  R@10={m['strict_i2t_R@10']:.4f}  |  "
                f"template R@1={m['template_i2t_R@1']:.4f}  R@5={m['template_i2t_R@5']:.4f}  R@10={m['template_i2t_R@10']:.4f}"
            )
    else:
        print(f"WARN: v1 joint embeddings not found at {V1_JOINT_NPZ} — comparison table will be v2-only")

    # ---- markdown report ----
    L = []
    L.append("# Step 4R-C clinical v2 — Contrastive Projection 학습 결과 (POSTURE_POOL + JOINT_POOL)")
    L.append("")
    L.append("- 생성 스크립트: `scripts/train_step4rc_contrastive_clinical_v2.py`")
    ...
    L.append(f"- input: imu (9275, 128) + text_clinical_v2 (9275, 768)")
    L.append(f"- corpus unique phrases: {corpus['phrase'].nunique()} (v1: 1733)")
    L.append(f"- corpus unique template_key: {len(np.unique(template_keys))}")
    L.append("")
    L.append("---")
    L.append("")
    L.append("## 1. v2 final retrieval metrics (best checkpoint)")
    L.append("")
    L.append("| split | strict R@1 | strict R@5 | strict R@10 | template R@1 | template R@5 | template R@10 |")
    L.append("|---|---:|---:|---:|---:|---:|---:|")
    for s_name in ["train", "val", "test"]:
        m = final_metrics_v2[s_name]
        L.append(
    ...
    if final_metrics_v1 is not None:
        L.append("## 2. v1 vs v2 비교 — i2t retrieval")
        L.append("")
        L.append("### 2.1 strict R@K (정확한 sample 매칭)")
        L.append("")
        L.append("| split | v1 R@1 | v2 R@1 | Δ R@1 | v1 R@5 | v2 R@5 | Δ R@5 | v1 R@10 | v2 R@10 | Δ R@10 |")
        L.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
        for s in ["train", "val", "test"]:
            v1m = final_metrics_v1[s]
            v2m = final_metrics_v2[s]
            row = (
                f"| {s} "
                f"| {v1m['strict_i2t_R@1']:.4f} | {v2m['strict_i2t_R@1']:.4f} | {v2m['strict_i2t_R@1'] - v1m['strict_i2t_R@1']:+.4f} "
                f"| {v1m['strict_i2t_R@5']:.4f} | {v2m['strict_i2t_R@5']:.4f} | {v2m['strict_i2t_R@5'] - v1m['strict_i2t_R@5']:+.4f} "
                f"| {v1m['strict_i2t_R@10']:.4f} | {v2m['strict_i2t_R@10']:.4f} | {v2m['strict_i2t_R@10'] - v1m['strict_i2t_R@10']:+.4f} |"
            )
            L.append(row)
        L.append("")
        ...
        for s in ["train", "val", "test"]:
            ...
            row = (
                f"| {s} "
                f"| {v1m['template_i2t_R@1']:.4f} | {v2m['template_i2t_R@1']:.4f} | {v2m['template_i2t_R@1'] - v1m['template_i2t_R@1']:+.4f} "
                f"| {v1m['template_i2t_R@5']:.4f} | {v2m['template_i2t_R@5']:.4f} | {v2m['template_i2t_R@5'] - v1m['template_i2t_R@5']:+.4f} "
                f"| {v1m['template_i2t_R@10']:.4f} | {v2m['template_i2t_R@10']:.4f} | {v2m['template_i2t_R@10'] - v1m['template_i2t_R@10']:+.4f} |"
            )
            L.append(row)
        L.append("")
        L.append("### 2.3 t2i 방향 (text → IMU) — template R@K")
        L.append("")
        L.append("| split | v1 R@1 | v2 R@1 | Δ R@1 | v1 R@5 | v2 R@5 | Δ R@5 | v1 R@10 | v2 R@10 | Δ R@10 |")
        L.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
        for s in ["train", "val", "test"]:
            ...
            row = (
                f"| {s} "
                f"| {v1m['template_t2i_R@1']:.4f} | {v2m['template_t2i_R@1']:.4f} | {v2m['template_t2i_R@1'] - v1m['template_t2i_R@1']:+.4f} "
                f"| {v1m['template_t2i_R@5']:.4f} | {v2m['template_t2i_R@5']:.4f} | {v2m['template_t2i_R@5'] - v1m['template_t2i_R@5']:+.4f} "
                f"| {v1m['template_t2i_R@10']:.4f} | {v2m['template_t2i_R@10']:.4f} | {v2m['template_t2i_R@10'] - v1m['template_t2i_R@10']:+.4f} |"
            )
            L.append(row)
        L.append("")
    ...
    print()
    print("=" * 64)
    print(f"Done. v2 best epoch={chk['epoch']}  val template R@1={best_val_template_R1:.4f}")
    if final_metrics_v1 is not None:
        v1_test = final_metrics_v1["test"]["template_i2t_R@1"]
        v2_test = final_metrics_v2["test"]["template_i2t_R@1"]
        print(f"        test template R@1: v1={v1_test:.4f}  v2={v2_test:.4f}  Δ={v2_test - v1_test:+.4f}")
    print("=" * 64)
    return 0
