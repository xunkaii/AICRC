"""Step 4R-C clinical v4 — Contrastive projection learning on v4 corpus.

Mirror of ``train_step4rc_contrastive_clinical.py`` (v1) — 모든 hyperparameter
동일. corpus 입력만 swap. 학습 종료 후 v1 / v3 joint embeddings 를 추가
로드해서 R@1/R@5/R@10 비교표 생성.
...
Reads (read-only):
    ...
    data/step4r/4rc_contrastive_optional/joint_embeddings_clinical.npz     (v1, 비교)
    data/step4r/4rc_contrastive_optional/joint_embeddings_clinical_v3.npz  (v3, 비교)
...
Run:
    & C:\\Users\\user\\anaconda3\\envs\\dl_env\\python.exe -X utf8 \\
        scripts/train_step4rc_contrastive_clinical_v4.py
"""
...
INPUT_TEXT = PROJECT_ROOT / "data" / "step4r" / "4rc_contrastive_optional" / "text_embeddings_clinical_v4.npz"
INPUT_CORPUS = PROJECT_ROOT / "data" / "step4r" / "4rc_contrastive_optional" / "text_corpus_clinical_v4.csv"
V1_JOINT_NPZ = PROJECT_ROOT / "data" / "step4r" / "4rc_contrastive_optional" / "joint_embeddings_clinical.npz"
V3_JOINT_NPZ = PROJECT_ROOT / "data" / "step4r" / "4rc_contrastive_optional" / "joint_embeddings_clinical_v3.npz"

OUTPUT_CKPT_DIR = PROJECT_ROOT / "data" / "step4r" / "4rc_contrastive_optional" / "model_clinical_v4"
...
OUTPUT_CKPT = OUTPUT_CKPT_DIR / "projection_head_clinical_v4.pt"
OUTPUT_JOINT_NPZ = OUTPUT_DATA_DIR / "joint_embeddings_clinical_v4.npz"
OUTPUT_LOG_CSV = OUTPUT_REPORT_DIR / "training_log_clinical_v4.csv"
OUTPUT_REPORT_MD = OUTPUT_REPORT_DIR / "results_clinical_v4.md"
...
def _eval_existing(npz_path: Path, label: str, expected_sid):
    if not npz_path.exists():
        return None
    d = np.load(npz_path, allow_pickle=True)
    sid = d["sample_id"].astype(str)
    spl = d["split"].astype(str)
    imu = d["imu_proj"]
    txt = d["text_proj"]
    tpl = d["template_key"].astype(str)
    assert (sid == expected_sid).all(), f"{label} sample_id mismatch"
    out = {}
    for s_name in ["train", "val", "test"]:
        idx = np.where(spl == s_name)[0]
        m = compute_retrieval(imu[idx], txt[idx], tpl[idx], k_list=(1, 5, 10))
        out[s_name] = m
        print(f"  {label}  {s_name:5s}  strict R@1={m['strict_i2t_R@1']:.4f}  R@5={m['strict_i2t_R@5']:.4f}  R@10={m['strict_i2t_R@10']:.4f}  |  "
...
def main() -> int:
    print("=" * 64)
    print("Step 4R-C clinical v4 — contrastive projection learning")
    print("=" * 64)
    ...
    for s_name, s_idx in [("train", train_idx), ("val", val_idx), ("test", test_idx)]:
        ...
        final_metrics_v4[s_name] = m
        print(f"  v4  {s_name:5s}  strict R@1={m['strict_i2t_R@1']:.4f}  R@5={m['strict_i2t_R@5']:.4f}  R@10={m['strict_i2t_R@10']:.4f}  |  "
              f"template R@1={m['template_i2t_R@1']:.4f}  R@5={m['template_i2t_R@5']:.4f}  R@10={m['template_i2t_R@10']:.4f}")
    ...
    # v1, v3 비교 metric 재계산
    print(f"\nloading v1 joint embeddings ...")
    final_metrics_v1 = _eval_existing(V1_JOINT_NPZ, "v1", sample_id)
    print(f"\nloading v3 joint embeddings ...")
    final_metrics_v3 = _eval_existing(V3_JOINT_NPZ, "v3", sample_id)

    # markdown report
    L = []
    L.append("# Step 4R-C clinical v4 — Contrastive Projection 학습 결과 (CONFIDENCE_POOL 교체)")
    L.append("")
    L.append("- 생성 스크립트: `scripts/train_step4rc_contrastive_clinical_v4.py`")
    ...
    L.append("---")
    L.append("")
    L.append("## 1. v4 final retrieval metrics (best checkpoint)")
    L.append("")
    L.append("| split | strict R@1 | strict R@5 | strict R@10 | template R@1 | template R@5 | template R@10 |")
    L.append("|---|---:|---:|---:|---:|---:|---:|")
    for s in ["train", "val", "test"]:
        m = final_metrics_v4[s]
        L.append(f"| {s} | {m['strict_i2t_R@1']:.4f} | {m['strict_i2t_R@5']:.4f} | {m['strict_i2t_R@10']:.4f} | "
                 f"{m['template_i2t_R@1']:.4f} | {m['template_i2t_R@5']:.4f} | {m['template_i2t_R@10']:.4f} |")
    L.append("")

    # v1 vs v3 vs v4 비교
    if final_metrics_v1 is not None and final_metrics_v3 is not None:
        L.append("## 2. v1 vs v3 vs v4 비교 — i2t retrieval")
        L.append("")
        for metric_name, key in [
            ("template R@1", "template_i2t_R@1"),
            ("template R@5", "template_i2t_R@5"),
            ("template R@10", "template_i2t_R@10"),
            ("strict R@1", "strict_i2t_R@1"),
            ("strict R@5", "strict_i2t_R@5"),
            ("strict R@10", "strict_i2t_R@10"),
        ]:
            L.append(f"### 2.{key} — {metric_name}")
            L.append("")
            L.append("| split | v1 | v3 | v4 | Δ(v4-v1) | Δ(v4-v3) |")
            L.append("|---|---:|---:|---:|---:|---:|")
            for s in ["train", "val", "test"]:
                a = final_metrics_v1[s][key]
                b = final_metrics_v3[s][key]
                c = final_metrics_v4[s][key]
                L.append(f"| {s} | {a:.4f} | {b:.4f} | {c:.4f} | {c-a:+.4f} | {c-b:+.4f} |")
            L.append("")
    ...
    L.append("")
    L.append("- v1: `scripts/train_step4rc_contrastive_clinical.py`, `joint_embeddings_clinical.npz`, `results_clinical.md`")
    L.append("- v2: 동일 pattern")
    L.append("- v3: 동일 pattern")
    L.append("")
    ...
    print()
    print("=" * 64)
    print(f"Done. v4 best epoch={chk['epoch']}  val template R@1={best_val_template_R1:.4f}")
    if final_metrics_v1 is not None:
        print(f"        test template R@1: v1={final_metrics_v1['test']['template_i2t_R@1']:.4f}  "
              f"v3={final_metrics_v3['test']['template_i2t_R@1']:.4f}  "
              f"v4={final_metrics_v4['test']['template_i2t_R@1']:.4f}")
    print("=" * 64)
    return 0
