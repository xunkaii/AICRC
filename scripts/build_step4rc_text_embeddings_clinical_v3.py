"""Step 4R-C clinical v3 — Frozen Korean SBERT embeddings on v3 corpus.

Mirror of ``build_step4rc_text_embeddings_clinical.py`` (v1). Only paths differ.

Reads (read-only):
    data/step4r/4rc_contrastive_optional/text_corpus_clinical_v3.csv

Writes (new files only):
    data/step4r/4rc_contrastive_optional/text_embeddings_clinical_v3.npz
    data/step4r/4rc_contrastive_optional/text_unique_similarity_clinical_v3.csv

기존 v1 / v2 산출물 보존.

Run:
    & C:\\Users\\user\\anaconda3\\envs\\dl_env\\python.exe -X utf8 \\
        scripts/build_step4rc_text_embeddings_clinical_v3.py
"""
...
INPUT_CSV = (
    ...
    / "text_corpus_clinical_v3.csv"
)
OUTPUT_DIR = PROJECT_ROOT / "data" / "step4r" / "4rc_contrastive_optional"
OUTPUT_NPZ = OUTPUT_DIR / "text_embeddings_clinical_v3.npz"
OUTPUT_SIM_CSV = OUTPUT_DIR / "text_unique_similarity_clinical_v3.csv"
...
def main() -> int:
    print("=" * 64)
    print("Step 4R-C clinical v3 — frozen ko-sroberta-multitask embeddings")
    print("=" * 64)
    ...
    print(f"loaded n={len(df)} rows from text_corpus_clinical_v3.csv")
