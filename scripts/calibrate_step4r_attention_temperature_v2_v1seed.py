"""Temperature scaling under parametrized split seed (v1-ratio robustness).

Wrapper around calibrate_step4r_attention_temperature_v2.py. Reuses helper
functions; defines a new main() that points to data/step4r_v1seed_robustness/
s{SEED}/ tree.

Run:
    & 'C:\\Users\\user\\anaconda3\\envs\\aicrc_env\\python.exe' -X utf8 \
        scripts/calibrate_step4r_attention_temperature_v2_v1seed.py \
        --split-seed 7 --seed 42 --exp-id b01_5_aug_jitter_scale_strong
"""
...
def main(argv=None) -> int:
    ...
    print("=" * 64)
    print(
        f"v1seed calibrate | split_seed={args.split_seed} "
        f"exp_id={args.exp_id} seed={args.seed}"
    )
    print("=" * 64)

    rel = Path("experiments") / args.exp_id / f"seed{args.seed}"
    root_data = (
        PROJECT_ROOT / "data" / "step4r_v1seed_robustness"
        / f"s{args.split_seed}"
    )
    root_report = (
    ...
    ckpt = root_ckpt / "4rb_attention" / rel / "best.pt"
    input_npz = root_data / "4rb_attention" / "step4r_sequence_dataset.npz"
    ...
    for s in base.SPLITS:
        ...
        print(
            ...
            f"macroF1={b['macro_f1']:.4f}  ECE={b['ece_15bin']:.4f}"
        )
        print(
            f"  {s:5s} after   acc={a['accuracy']:.4f}  "
            f"macroF1={a['macro_f1']:.4f}  ECE={a['ece_15bin']:.4f}"
        )
    ...
    L.append(
        f"# v1seed calibrate | split_seed={args.split_seed} "
        f"exp=`{args.exp_id}` seed=`{args.seed}`"
    )
    L.append("")
